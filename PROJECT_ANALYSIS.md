...
## Base Paper Method Integration & Empirical Comparison

### 1. Reference Paper Details
- **Title**: *"Fast and Cost-Aware Workload Prediction for Precised Auto-scaling Using Novel Knowledge Distillation Technique"* (SSRN Preprint / FGCS 2024).
- **Authors**: Sharmen Akhter, Md Imtiaz Hossain, Nosin Ibna Mahbub, Eui-Nam Huh (Kyung Hee University).
- **Core Contribution**: Proposed **Filter-KD**, a novel Knowledge Distillation technique for regression tasks in cloud workload forecasting.

### 2. Mathematical Formulation of Filter-KD
Filter-KD restricts erroneous teacher predictions from propagating to the lightweight student network during training:
1. **Teacher Model ($\Psi^T$)**: Stacked Bi-LSTM model (e.g. 512x2 / 512x4 units).
2. **Student Model ($\Psi^S$)**: Lightweight Bi-LSTM model (e.g. 128x2 units).
3. **Filter-KD Loss Function**:
   $$L_K = \frac{1}{m} \sum_{i=1}^m \left[ \gamma \|\Psi^S(x_i) - y_i\|^2 + (1-\gamma) L_{\text{filter}, i} \right]$$
   where:
   $$L_{\text{filter}, i} = \begin{cases} \|\Psi^S(x_i) - \Psi^T(x_i)\|^2, & \text{if } \|\Psi^T(x_i) - y_i\|^2 \le \epsilon \\ 0, & \text{otherwise} \end{cases}$$
   - $\gamma$: Balancing parameter between ground-truth MSE and teacher KD loss.
   - $\epsilon$: Error tolerance threshold filtering out noisy teacher outputs.

### 3. Empirical Benchmarking on Project Dataset
To evaluate the improvements introduced by our **AutoScale Intelligence AI** architecture, both the Base Paper Baseline (Bi-LSTM-128x2), Base Paper Proposed (Filter-KD Bi-LSTM), and our Improved Model (**TCN + Attention Ensemble**) were trained and benchmarked on identical 12-timestep sequence data:

- **Baseline Bi-LSTM**: Standard recurrent architecture with MSE loss.
- **Filter-KD Bi-LSTM**: Distilled student model benefiting from $\epsilon$-filtered teacher representation transfer.
- **TCN + Attention Ensemble (Our Model)**: Combines causal 1D dilated convolutions with temporal attention and 5-model deep ensemble variance for uncertainty estimation.

All comparative graphs (Learning Curves, Actual vs Predicted Overlay, and Metric Bar Charts) are automatically generated by `train_and_compare_base_paper.py` and displayed inline in the React Frontend UI (`BasePaperComparison.jsx`).
