      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-border/50 pb-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Toggle model lines ON/OFF or switch between 2x3 Grid View and Individual Architecture focus.
          </p>
        </div>

        {/* 3 Interactive Model Line Toggle Buttons */}
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            Our Improved Model (Blue)
          </button>
        </div>
      </div>

      {/* View Selector Tabs (Grid vs Individual Architectures) */}
                ...
                <div className="h-56 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={item.series} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#161B22', borderColor: '#30363D', color: '#fff', fontSize: '12px' }} />
        ...
        <div className="bg-card/40 border border-primary/30 p-6 rounded-xl flex flex-col gap-4">
          ...
          <div className="h-96 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={graphsData[activeArch].series} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#30363D" />
                <XAxis dataKey="epoch" label={{ value: 'Epochs', position: 'insideBottom', offset: -5, fill: '#9CA3AF' }} stroke="#6B7280" />
                <YAxis label={{ value: 'Error / Loss (MSE)', angle: -90, position: 'insideLeft', fill: '#9CA3AF' }} stroke="#6B7280" />
                ...
                {showBaseline && (
                  <Line
                    type="monotone"
                    dataKey="baseline"
                    name="Base Paper Baseline (Standard Bi-LSTM)"
                    stroke="#EF4444"
                    strokeDasharray="4 4"
                    strokeWidth={2.5}
                  />
                ...
                {showFilterKD && (
                  <Line
                    type="monotone"
                    dataKey="filter_kd"
                    name="Base Paper Filter-KD (Distilled Student)"
                    stroke="#F59E0B"
                    strokeWidth={2.5}
                  />
                )}
                {showOurModel && (
                  <Line
                    type="monotone"
                    dataKey="our_model"
                    name="Our Improved Model (TCN + Attention Ensemble)"
                    stroke="#3B82F6"
                    strokeWidth={3}
                  />
                )}
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t border-border/40 text-center">
            <div className="bg-red-500/10 p-3 rounded-lg border border-red-500/20">
              <div className="text-xs text-red-400 font-medium">Baseline MSE</div>
            ...
            <div className="bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
              <div className="text-xs text-amber-400 font-medium">Filter-KD MSE</div>
            ...
            <div className="bg-blue-500/10 p-3 rounded-lg border border-blue-500/20">
              <div className="text-xs text-blue-400 font-medium">Our Model MSE</div>
